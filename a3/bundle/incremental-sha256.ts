...
export class IncrementalSha256 {
  ...
  update(bytes: Uint8Array): this {
    ...
    while (cursor + 64 <= bytes.byteLength) {
      this.#compress(bytes, cursor);
      cursor += 64;
    }
    if (cursor < bytes.byteLength) {
      this.#buffer.set(bytes.subarray(cursor), 0);
      this.#buffered = bytes.byteLength - cursor;
    }
    return this;
  }
  ...
  digest(): Uint8Array {
    if (this.#finalized) throw new Error("SHA-256 digest is already finalized");
    const bitLength = (this.#byteLength * BigInt(8)) & ((BigInt(1) << BigInt(64)) - BigInt(1));
    const paddingLength = this.#buffered < 56 ? 64 - this.#buffered : 128 - this.#buffered;
    const padding = new Uint8Array(paddingLength);
    padding[0] = 0x80;
    new DataView(padding.buffer).setBigUint64(paddingLength - 8, bitLength, false);
    this.update(padding);
    this.#finalized = true;
