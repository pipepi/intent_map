...
import { assertElementManifest } from "./package-validation.ts";
import type { ElementPluginManifest, ElementPluginPackage } from "./package-types.ts";
...
export async function decodeElementPackage(archive: Uint8Array): Promise<ElementPluginPackage> {
  ...
  return { manifest, entrySource: strFromU8(files[manifest.entry]), files, archive };
}
