...
import { ExecutionEventQueue } from "../pip-editor/relation-host/execution/event-queue.ts";
import { ExecutionSessionManager } from "../pip-editor/relation-host/execution/session-manager.ts";
...
test("Intent execution follows the existing contains scopes down and returns a root output", async () => {
  ...
  assert.equal(suite.nodeMap.graph.revision, 0);
});
...
test("trigger validators reject private child targets", async () => {
  ...
  const target = graph.nodes["intent.trigger.manual"].relations.find((item) => item.id === "target");
  target.object = { kind: "ref", target: { nodeId: "intent.app-state", relationId: "identity" } };
  assert.throws(() => registry.validators().forEach((validate) => validate(graph)), /private child/);
});

test("execution results enter RelationGraph only through explicit persistence", async () => {
  ...
  const applied = applyRelationPatch(suite.nodeMap.graph, manager.persistencePatch(sessionId, 0));
  registry.validators().forEach((validate) => validate(applied.graph));
  assert.equal(applied.graph.revision, 1);
  assert.ok(Object.keys(applied.graph.nodes).some((id) => id.startsWith("relation.execution.run.")));
});
