...
import { exportedWorkspaceViews, normalizeFreeLayout, preserveSystemWindows } from "../pip-editor/relation-host/workspace/view-state.ts";
...
test("default camera exposes the world origin at the viewport corner", async () => {
  ...
  assert.equal(views.camera.scale, 1); assert.equal(views.camera.x, 0); assert.equal(views.camera.y, 0);
});
...
test("creator transaction adds a projection root and undo restores graph roots and views", async () => {
  ...
  assert.ok(normalizeFreeLayout(current.views, current.rootNodeIds).projections[source.id]); assert.equal(normalizeFreeLayout(current.views, current.rootNodeIds).activeWindowId, source.id);
...
test("reusing a blank tab for A5 preserves its local plugin manager", async () => {
  ...
  assert.ok(views.systemWindows["host.plugin-manager"]); assert.equal(views.activeWindowId, "host.plugin-manager");
});
