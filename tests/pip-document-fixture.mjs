...
const pip = (id, value) => ({ id, fork_level: 3, predicate_value: { predicate: identity, value: { kind: "const", value } }, pips: [] });
const ref = (id, nodeId) => ({ id, fork_level: 3, predicate_value: { predicate: identity, value: { kind: "ref", target: { node_id: nodeId, pip_id: "identity" } } }, pips: [] });
...
export const samplePipDocument = (depth = 1) => {
  ...
  for (let index = 0; index < depth; index += 1) {
    const id = index ? `sample.depth-${index + 1}` : "sample.root";
        ...
        nodes.push({ id, fork_level: 2, pips: [pip("identity", id), pip("name", `第 ${index + 1} 层`), ...(childId ? [ref("contains", childId)] : [])] });
    }
