...
import { decodeElementPackage } from "../pip-editor/pip-host/packages/element-package.ts";
import { decodeNodeTypePackage, validateNodeTypeDependencies } from "../pip-editor/pip-host/packages/node-type-package.ts";
...
import { projectionOptions } from "../pip-editor/pip-host/projection/projection-routes.ts";
...
test("spot creator installs equivalent terminal projections and real child projections", () => {
    const result = terminalCreator.create({ graph: createGraph({}, 7) });
    const nodes = result.patch.operations.filter((operation) => operation.op === "put" && operation.parent_path.length === 0).map((operation) => operation.pip);
    const uses = (node) => node.pips.find((pip) => pipStatement(pip).predicate.node_id === "pip.projection.predicate.uses")?.predicate_value?.value?.target?.node_id;
    ...
    assert.ok(root.pips.some((pip) => pipStatement(pip).predicate.node_id === "pip.projection.predicate.dives-into"));
    assert.equal(uses(nodes.find((node) => node.id === pipStatement(root.pips.find((pip) => pipStatement(pip).predicate.node_id === "pip.projection.predicate.dives-into")).value.target.node_id)), "spot.terminal.projection.children");
    const simple = nodes.find((node) => uses(node) === "spot.terminal.projection.simple");
    assert.equal(uses(nodes.find((node) => node.id === pipStatement(simple.pips.find((pip) => pipStatement(pip).predicate.node_id === "pip.projection.predicate.dives-into")).value.target.node_id)), "spot.terminal.projection.children");
    const flow = nodes.find((node) => uses(node) === "spot.terminal.projection.children");
    assert.equal(flow.pips.filter((pip) => pipStatement(pip).predicate.node_id === "pip.projection.predicate.presents").length, 1);
});
test("spot A4 runtime exposes equivalent terminal and child projections", async () => {
    ...
    const created = registry.creators().find(({ id }) => id === "spot.terminal.create")
        .create({ graph: createGraph({}, 0) });
    const nodes = Object.fromEntries(created.patch.operations
    .filter(({ op, parent_path }) => op === "put" && parent_path.length === 0).map(({ pip }) => [pip.id, pip]));
    const terminalId = pipStatement(nodes[created.preferredProjection.projectionId].pips
        .find(({ predicate_value }) => predicate_value.predicate.node_id === "pip.projection.predicate.observes")).value.target.node_id;
    assert.deepEqual(projectionOptions(terminalId, createGraph(nodes, 1), registry).map(({ scope, label }) => [scope, label]), [
        ["self", "↗ Spot Detail"],
        ["self", "▣ Spot Simple"],
        ["children", "⇄ Spot Flow"],
