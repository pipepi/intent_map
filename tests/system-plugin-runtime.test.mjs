...
import { SystemPluginRegistry } from "../pip-editor/pip-host-io/system-plugin/registry.ts";
import {
  createSystemPluginTypeNode,
  SystemPluginRuntime,
} from "../pip-editor/pip-host-io/system-plugin/runtime.ts";
...
const definition = ({
  ...
  id,
  typeNode: createSystemPluginTypeNode(`pip.host.type.${id}`),
  label: id,
  category: "系统",
  scope,
...
test("multiple instances are independent and release their overlay nodes", () => {
    ...
    assert.ok(graphNodes(runtime.snapshot().graph)[first.id]);
    assert.ok(graphNodes(runtime.snapshot().graph)[second.id]);
...
test("runtime releases host singletons and continues after disposal errors", () => {
    ...
    assert.equal(graphNodes(runtime.snapshot().graph)[failingInstance.id], undefined);
    assert.equal(graphNodes(runtime.snapshot().graph)[healthyInstance.id], undefined);
});
