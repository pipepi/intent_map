...
run("build-seed-source-pip.mjs");
run("build-seed-desktop.mjs");
run("build-loader-pip.mjs");
run("build-editor-static.mjs");
...
const seedSource = await readFile(systemPipFilePath(release.seed));
...
const receipt = {
  ...
  artifacts: await Promise.all([
    ...
    ...selected.map(async (item) => {
      const source = await readFile(systemPipFilePath(item));
