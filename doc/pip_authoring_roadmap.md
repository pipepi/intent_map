...
当前编辑器已经能够通过 A4 Creator 创建节点和 Projection Instance，并由 A3 展示和交互；但还没有形成完整的 Pip 编辑、节点类型创作和节点 UI 元素创作能力。
...
Pip Editor 不是新的专用节点类型，而是现有 A2 Editor 对 PipGraph 实例编辑能力的持续增强。新增、删除、改向和选择 Pip，都发生在用户当前正在观察的业务节点及 Projection 中；A2 协调手势和事务，A3 暴露视觉锚点，A4 提供语义规则，最终修改当前 A5。
...
| `official.pip-projection-elements` | `properties` | projection | 通用 self 属性 |
...
当前正式声明只有 `id + tag + purpose`；A4 Projection 只能硬编码一个 `pluginId + elementId`。注册表无法回答元素支持的 surface、视觉角色、输入模型、组合槽位、交互、Pip 锚点、布局约束或预览 fixture。因此这些元素能支持手写 A4，但还不能形成 Node Type Studio 可检索、选择和组合的元素语言。
...
Pattern     Property List、Node Card、Projection Collection、Pip Anchor 等模式
...
“完备”指每个定义维度完整，不表示预制所有未来业务 UI。A3 PIP 的 PipDocument 以 Library 为根，所有可独立引用和编辑的定义都必须是典型 PipNode，而不是隐藏在 manifest 或某个节点 const JSON 中：
...
交互能力：Field Editor、Action、Selection、Pip Anchor
结构能力：Section、Stack/Grid、Tabs、Empty/Error/Loading
```

组件库必须有目录、分类、搜索、交互预览、Props/Events/Slots API 表和复制/引用示例。A3 负责视图模型和视觉交互；A4 仍负责领域解释、predicate、端点合法性和 PipTx。
...
候选专用类型包括 element-project、element-definition、element-tree、style-token、interaction-binding 和 pip-anchor，统一位于 `pip.authoring.type.*` 命名空间。
...
候选专用类型包括 node-type-project、node-type-definition、pip-definition、projection-definition、creator-definition 和 validator-definition，统一位于 `pip.authoring.type.*` 命名空间。
...
实现位置：增强现有 Pip Editor，不创建 Pip Editor 节点类型，也不要求用户先打开一个独立工具节点。
...
- 松开后由 A4 command 生成 revision-bound PipTx；
- 提交进入既有校验和 undo/redo。
...
最小供应：选中真实 Pip、调用 A4 语义命令、原子删除或替换，并保证嵌套关系和引用闭包合法。
...
最小供应：显式迁移命令、迁移前诊断、单次 PipTx、撤销或备份策略，以及旧版与新版 fixture。
...
4. 用第二种节点类型验证 Pip Definition、Projection 与 A3 绑定；
5. 在现有 A2 Editor 中完成单一 predicate 的 Pip 拖拽；
