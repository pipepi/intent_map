...
PIP Seed 是系统中唯一需要操作系统直接双击运行的原生文件。它是无状态、尽可能小且长期稳定的初始加载器。
...
Seed 与 `.pip` 始终是相互独立的文件。Seed 不内嵌默认应用，也不从自身可执行文件的尾部提取 PIP。

启动时按以下优先级确定目标：

1. 如果启动参数显式提供 `.pip` 路径，加载该文件；
2. 否则扫描 Seed 分发根目录的 `pip/`；
...
Seed 不拥有上层应用状态。应用退出后，种皮自身不保留业务状态。

## 与 Loader 的边界

Seed 只提供最小加载机制；文件选择、版本管理、更新、权限交互和恢复等持续变化的功能属于 [Loader](loader.md)。
...
因为 `pip/` 中只有一个默认有效 `a1` PIP，Seed 可以直接启动它。macOS
...
- `pip-seed/runtime/`：Seed CLI 与桌面宿主共享的 PIP 解析、发现、目录、Profile 和信任机制；
- `pip-seed/cli/`：最小命令行与 HTTP 宿主启动能力；
- `pip-seed/tauri/`：最小桌面 WebView 宿主；
- 与原生平台启动直接相关的少量构建逻辑。

实现时应持续检查：某个新功能是否真的必须原生存在。如果可以由 `.pip` 实现，就应上移到 Loader 或其他应用。
...
- Seed 可执行文件中不包含内嵌 `.pip`；
- 能拒绝损坏或不受支持的包；
- 上层 `.pip` 更新不要求同步修改 Seed；
- 原生依赖和平台权限保持最少。
