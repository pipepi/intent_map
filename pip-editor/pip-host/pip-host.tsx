...
import {
  ...
  type EditorPreferences} from "../pip-host-io/index.ts";
import type {
  ExecutionContextSnapshot,
  PipElementRequest,
  WorkspacePoint} from "./contracts/package-types.ts";
import { ExecutionSessionManager } from "./execution/session-manager.ts";
import { dispatchPipElementRequest } from "./pip-host-actions.ts";
import { createScratchWorkspace, downloadBlob } from "./pip-host-workspace.ts";
...
import { PipHostSurface } from "./view/pip-host-surface.tsx";
import { buildWorkspaceExport } from "./workspace-export.ts";
import {
  createHostCanvasState,
  type HostViewportSize,
  HostPresentationStore} from "./workspace/host-presentation-store.ts";
import {
  graphFingerprint,
  WorkspaceSessionStore,
  type WorkspaceSession} from "./workspace/workspace-store.ts";
...
export function PipHost() {
  ...
  const handleRequest = async (
    workspace: WorkspaceSession,
    request: PipElementRequest,
  ) => {
    try {
      await dispatchPipElementRequest(request, {
        active: workspace,
        commandQueues,
        executionManager,
