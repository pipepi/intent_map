...
const typeKey = (type: ResolvedNodeType["type"]) => `${type.node_id}\u0000${type.pip_id}`;
...
export class NodeTypePluginRegistry {
    ...
    readonly #types = new Map<string, {
        pluginId: string;
        value: ResolvedNodeType;
    }>();
    readonly #validators = new Set<PipValidator>();
    readonly #commands = new Map<string, {
        pluginId: string;
        value: PipCommandHandler;
    }>();
    readonly #executors = new Map<string, {
        pluginId: string;
        value: PipExecutor;
    }>();
    readonly #projections = new Map<string, {
        pluginId: string;
        value: PipProjection;
    }>();
    readonly #languages = new Map<string, {
        pluginId: string;
        value: PipLanguageProvider;
    }>();
    readonly #creators = new Map<string, {
        pluginId: string;
        value: PipCreator;
    }>();
    readonly #planners = new Map<string, {
        pluginId: string;
        value: PipExecutionPlanner;
    }>();
    readonly #runtimes = new Map<string, {
        pluginId: string;
        value: PipNodeRuntime;
    }>();
    readonly #operators = new Map<string, {
        pluginId: string;
        value: PipOperator;
    }>();
    readonly #triggers = new Map<string, {
        pluginId: string;
        value: PipTriggerProvider;
    }>();
    readonly #effects = new Map<string, {
        pluginId: string;
        value: PipEffectHandler;
    }>();
    ...
    pipOperators() { return new Map([...this.#operators].map(([id, item]) => [id, item.value])); }
    ...
    async #installOnce(plugin: NodeTypePluginPackage): Promise<PluginInstallStatus> {
      ...
      registerPipOperator: (operator) => own(this.#operators, operator.id, operator, "Pip operator"),
      ...
      registerEffectHandler: (handler) => own(this.#effects, handler.type, handler, "Effect handler"),
        ...
        try {
            ...
            if (typeof loaded.default !== "function")
                throw new Error("Node type entry must default-export a register function");
            const pluginDisposable = loaded.default(host);
            if (pluginDisposable)
                disposables.push(() => dispose(pluginDisposable));
            const expectedTypes = new Set(plugin.manifest.typeNodeIds.map((nodeId) => typeKey({ node_id: nodeId, pip_id: "identity" })));
            ...
            if (cleanupErrors.length)
                throw new AggregateError([error, ...cleanupErrors], `Node type plugin ${plugin.manifest.packageId} failed and cleanup reported errors`);
            throw error;
        }
    }
    disable(id: string) {
