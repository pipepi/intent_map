...
import type { ExecutionContextSnapshot, ProjectionNavigationState, PipElementRequest, WorkspacePoint } from "../contracts/package-types.ts";
...
import { PipNodeRenderer } from "./projection-renderer.tsx";
...
export function SemanticProjection({ workspace, workspaceView, rootWindowId, navigation, contentOffset, execution, elements, nodeTypes, selection, onRequest }: {
  ...
  workspaceView: JsonValue; contentOffset: WorkspacePoint; nodeTypes: NodeTypePluginRegistry; selection: string[]; onRequest: (request: PipElementRequest) => void;
  execution?: ExecutionContextSnapshot;
}) {
  const viewport = useRef<HTMLDivElement>(null), [flip, setFlip] = useState<FlipFrame>();
  const route = currentRoute(navigation), current = graphNodes(workspace.graph)[route.projectionNodeId];
  ...
  const semanticOrigin = navigation.semanticOrigin, pointerOrigin = semanticOrigin ? `${semanticOrigin.x}px ${semanticOrigin.y}px` : "center";
    ...
    if (!current)
        return <div className={styles.orphan}>导航目标 {route.projectionNodeId} 不存在</div>;
    ...
    const render = (entry: typeof route, opacity: number, target = false) => {
        const node = graphNodes(workspace.graph)[entry.projectionNodeId];
        if (!node)
            return null;
      ...
      <PipNodeRenderer workspaceId={workspace.id} rootNodeIds={workspace.rootNodeIds} workspaceView={workspaceView} graph={workspace.graph} node={node} selection={selection} purpose="workspace" projectionContext={workspaceProjectionContext(entry.scope)} execution={execution} elements={elements} nodeTypes={nodeTypes} onRequest={onRequest}/>
    </div></div>;
    };
