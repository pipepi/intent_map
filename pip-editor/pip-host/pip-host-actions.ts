...
/** PipHost 的领域动作：串行执行命令，并分发 A3 发出的宿主请求。 */
import type { JsonValue, PipRef } from "../pip/index.ts";
 ...
 * 同时基于同一个 revision 生成互相冲突的 PipTx。
 */
async function runCommand(
  request: Extract<PipElementRequest, { kind: "command" }>,
  ...
  const queued = previous.catch(() => undefined).then(async () => {
    ...
    if (!command) {
      throw new Error(`Unknown pip command ${request.commandId}`);
    }
...
export async function dispatchPipElementRequest(request: PipElementRequest, context: DispatchContext) {
    const {
    active,
    executionManager,
    invokeCreator,
    ...
    switch (request.kind) {
        ...
        case "navigate-projection": {
            ...
            if (!windowId || !frame) {
                throw new Error("Projection navigation requires an active workspace window");
            }
        ...
        frame.navigation,
            ...
            const parent = route
                ? graphNodes(active.graph)[route.projectionNodeId]
                : undefined;
        ...
        : false;
            if (!navigation ||
                ...
                !parent ||
                !isPresentedChild) {
                throw new Error("Projection is not a direct child of the active internal view");
            }
        ...
        case "persist-execution-result":
            workspaceStore.commitPatch(workspaceId, executionManager.persistencePatch(request.sessionId, graphRevision(active.graph)), nodeTypes.validators());
    }
}
