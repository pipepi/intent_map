...
返回可选 `PipTx`、root 增量和首选窗口尺寸，宿主把它们作为一个事务校验、
提交并记录历史。

## 系统插件

`../pip-host-io/` 保存随 A2 编译的可信系统插件。系统 type node 和 instance
node 是独立系统 GRAPH Pip overlay 中的 `PipForkLevel.NODE` Pip，不属于业务 Graph
