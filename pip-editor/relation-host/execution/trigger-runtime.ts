...
import type { RelationTriggerProvider } from "../contracts/package-types.ts";

type WorkspaceGraph = { id: string; graph: RelationGraph };
...
type TriggerActivation = Exclude<ReturnType<NonNullable<RelationTriggerProvider["activation"]>>, undefined>;
type ActiveTrigger = {
  key: string; signature: string; workspace: WorkspaceGraph; triggerNodeId: string;
  provider: RelationTriggerProvider; timer?: ReturnType<typeof setInterval>; hookKey?: string;
};
...
export class TriggerRuntime {
  ...
  sync(workspaces: WorkspaceGraph[]) {
    ...
    for (const workspace of workspaces) for (const trigger of Object.values(workspace.graph.nodes)) {
      ...
      const key = `${workspace.id}\u0000${trigger.id}`, signature = JSON.stringify([workspace.graph.revision, activation]);
    ...
    for (const [key, current] of this.#active) if (!desired.has(key) || desired.get(key)?.signature !== current.signature) this.#remove(key);
    for (const [key, item] of desired) if (!this.#active.has(key)) {
      if (item.activation.kind === "hook") this.#active.set(key, item);
      else {
  ...
  async dispatchHook(hookKey: string, payload: JsonValue) {
    ...
    if (!targets.length) throw new Error(`Unknown relation hook ${hookKey}`);
