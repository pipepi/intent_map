...
目录按一次交互经过的机制环节组织，新读者可以沿同一方向追踪状态演进：

```text
contracts → packages → activation
                         ↓
DOM request → relation-host → workspace → RelationPatch → RelationGraph
                         ↓
                    projection → view
```

- `contracts/` 定义插件与宿主共同遵守的 ABI，并在信任输入前验证 JSON。
- `packages/` 解码、校验和标识 ZIP 包，不激活其中的能力。
- `activation/` 按依赖顺序激活 Collection，并维护 Element 与 Node Type registry。
- `workspace/` 持有会话状态，通过原子 Patch、undo/redo 推进关系图。
- `projection/` 从关系图纯计算展示匹配，并向 Web Component 桥接只读 context。
- `view/` 只负责工作区画布、插件面板和样式。
- `relation-host.tsx` 是总入口，串联安装、request、command、patch 与状态发布。
