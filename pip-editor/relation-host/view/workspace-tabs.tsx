...
export function WorkspaceTabs({
  ...
  >
    {workspaces.map((workspace) => {
      ...
      >
        <button
          ...
          title={`${name} · ${workspace.source.id}@${workspace.source.version}`}
          onClick={(event) => {
            event.currentTarget.blur();
            onActivate(workspace.id);
          }}
          onAuxClick={(event) => {
            if (event.button === 1) onClose(workspace.id);
          }}
        >
          <span>{name}</span>
          <small>{workspace.source.id === "host.new-tab"
            ? ""
            : `r${workspace.graph.revision}`}</small>
          <i
            role="button"
            aria-label={`关闭 ${name}`}
    ...
    <button
      type="button"
      className={styles.workspaceTabAdd}
      onClick={(event) => {
        event.currentTarget.blur();
        onNew();
      }}
