...
import type { ProjectionNavigationState } from "../contracts/package-types.ts";
import { currentRoute, moveProjectionHistory, navigateProjection } from "../projection/projection-navigation.ts";
...
export function ProjectionNavbar({ navigation, resizeMode, graph, nodeTypes, onChange, onReset, onClose }: {
  ...
  const changeProjection = (projectionNodeId: string) => {
    const next = routeForProjection(projectionNodeId, graph, nodeTypes); if (next) onChange(navigateProjection(navigation, next));
  };
