...
export async function decodeNodeTypePackage(pipBytes: Uint8Array, options?: PipIoOptions): Promise<NodeTypePluginPackage> {
  ...
  for (const asset of pip.assets.filter(({ path }) => path.startsWith("packages/"))) {
    ...
    if (asset.path !== `packages/${pipFilename(element.manifest)}`) throw new Error(`Embedded PIP filename does not match manifest: ${asset.path}`);
    const ref = manifest.dependencies.find((item) => item.packageId === element.manifest.packageId);
    if (!ref || ref.version !== element.manifest.packageVersion || ref.releaseDate !== element.manifest.releaseDate || ref.sha256 !== element.contentSha256) throw new Error(`内嵌 A3 内容身份不匹配 ${element.manifest.packageId}`);
