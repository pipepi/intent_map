...
export const assertPipManifest = (manifest: PipManifest): PipManifest => {
  ...
  const raw_manifest = manifest as unknown as Record<string, unknown>;
  if (raw_manifest.elementAbi === "relation-element/2" || raw_manifest.nodeTypeAbi === "relation-node-type/2") {
    throw new Error("Legacy executable plugin must be rebuilt for Pip（旧可执行插件需要重建）");
  }
...
const validElementManifest = (manifest: Extract<PipManifest, { layer: "a3" }>) =>
  manifest.elementAbi === "pip-element/2" && manifest.entry === "entry.mjs" && manifest.providedCapabilities.length > 0 &&
...
const validNodeTypeManifest = (manifest: Extract<PipManifest, { layer: "a4" }>) =>
  manifest.nodeTypeAbi === "pip-node-type/2" && manifest.entry === "entry.mjs" && manifest.typeNodeIds.length > 0 &&
...
const validNodeMapManifest = (manifest: Extract<PipManifest, { layer: "a5" }>) =>
  ["pip-node-map/1", "relation-node-map/1"].includes(manifest.nodeMapAbi) && Array.isArray(manifest.rootNodeIds) && manifest.rootNodeIds.every(Boolean) &&
