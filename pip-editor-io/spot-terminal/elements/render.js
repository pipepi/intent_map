...
export function loginView(state) {
  const keys = "1234567890".split("").map((key) => `<button type="button" data-login-key="${key}">${key}</button>`).join("");
  return `<section class="login"><form class="login-card" data-action="login" autocomplete="off">${environmentToggle(state)}<h1>AEX Spot Terminal</h1><p>登录当前选择的交易环境；切换环境不会携带账号或凭证。</p><label>用户名<span class="field"><input name="username" autocomplete="off" autocapitalize="none" spellcheck="false" required placeholder="member username"><button type="button" data-login-field="username">键盘</button></span></label>${state.environment === "server" ? `<label>密码<span class="field"><input name="password" type="password" autocomplete="current-password" required placeholder="account password"><button type="button" data-login-field="password">键盘</button></span></label>` : ""}<div class="login-keypad">${keys}<button type="button" data-login-key="backspace">⌫</button><button type="button" data-login-key="clear">清空</button></div>${state.error ? `<div class="error">${esc(state.error)}</div>` : ""}${state.notice ? `<div class="notice">${esc(state.notice)}</div>` : ""}<button class="primary" ${state.loading ? "disabled" : ""}>${state.loading ? "登录中…" : "登录"}</button><small>凭证仅保存在当前插件运行内存中</small></form></section>`;
}
