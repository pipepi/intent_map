import { cancelOrder, draft, login, logout, selectSymbol, setBotSide, setBotType, submitOrder, sync, toggleBot } from "./commands.js";
...
export default function register(host) {
  ...
  [["login", login], ["select-symbol", selectSymbol], ["submit", submitOrder], ["cancel", cancelOrder], ["sync", sync], ["draft", draft], ["bot-toggle", toggleBot], ["bot-side", setBotSide], ["bot-type", setBotType], ["logout", logout]].forEach(([name, command]) => releases.push(host.registerCommand(`spot.terminal.${name}`, command)));
