...
const definition_of = (node) => target(node, "pip.projection.predicate.uses")?.node_id;
const type_of = (node) => target(node, "pip.core.type")?.node_id;
const observed = (projection_node, graph) => graphNodes(graph)[target(projection_node, "pip.projection.predicate.observes")?.node_id];
const options_for = (graph, observed_node_id, current_projection) => {
  const dive_target = target(current_projection, "pip.projection.predicate.dives-into")?.node_id;
  return Object.values(graphNodes(graph)).flatMap((node) => {
    if (target(node, "pip.projection.predicate.observes")?.node_id !== observed_node_id) return [];
...
const composition_items = (projection_node, graph) => projection_node.pips.flatMap((pip) => {
    if (pip.predicate_value?.predicate.node_id !== "pip.projection.predicate.presents" || pip.predicate_value.value.kind !== "ref")
        return [];
    const child_projection = graphNodes(graph)[pip.predicate_value.value.target.node_id], child = child_projection && observed(child_projection, graph);
    const frame_pip = pip.pips.find((item) => item.predicate_value?.predicate.node_id === "pip.projection.predicate.frame");
    return child && frame_pip?.predicate_value?.value.kind === "const" ? [{ projectionNodeId: child_projection.id, observedNodeId: child.id, frame: frame_pip.predicate_value.value.value }] : [];
});
const projection = (id, name, scope, surfaces, element_id, project) => ({
    ...
    purpose: "workspace", definition: { node_id: id, pip_id: "identity" }, scope, surfaces,
...
export default function register(host) {
    const releases = [];
    releases.push(host.registerType({ type: { node_id: "spot.terminal.type.workspace", pip_id: "identity" }, name: "Spot Terminal",
    ...
    for (const kind of fact_types)
        releases.push(host.registerType({ type: { node_id: `spot.terminal.type.${kind}`, pip_id: "identity" }, name: kind,
    ...
    for (const [kind, name] of [["flow", "Spot Flow"], ["world", "Spot World Events"]])
        releases.push(host.registerProjection(projection(definitions[kind], name, "children", ["workspace"], "composition", ({ projectionNode, graph }) => ({ viewKind: kind, items: composition_items(projectionNode, graph) }))));
    for (const kind of fact_types)
        for (const style of ["simple", "detail"])
            releases.push(host.registerProjection(projection(
    ...
    for (const [id, key] of command_names)
        releases.push(host.registerCommand(`spot.terminal.${id}`, commands[key]));
