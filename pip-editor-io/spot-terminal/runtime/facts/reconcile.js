...
const frame_of = (relation) => {
  const nested = relation?.relations.find((item) => item.predicate.nodeId === "relation.projection.predicate.frame");
  return nested?.object.kind === "const" && valid_frame(nested.object.value) ? nested.object.value : undefined;
};
const overlaps = (left, right) => left.x < right.x + right.width && left.x + left.width > right.x
  && left.y < right.y + right.height && left.y + left.height > right.y;

const projection_by_definition = (graph, terminal_id, definition_id) => Object.values(graph.nodes).find((node) =>
  node.relations.some((item) => item.predicate.nodeId === "relation.projection.predicate.observes" && item.object.kind === "ref" && item.object.target.nodeId === terminal_id)
  && node.relations.some((item) => item.predicate.nodeId === "relation.projection.predicate.uses" && item.object.kind === "ref" && item.object.target.nodeId === definition_id));
const fact_kind = (node) => node.relations.find((item) => item.predicate.nodeId === "relation.core.type" && item.object.kind === "ref")?.object.target.nodeId.replace("spot.terminal.type.", "");

const composed_projection = (source, facts, mode) => {
  const targets = new Set(facts.map((node) => projection_id(node.id, "simple")));
  const current = new Map(source.relations.flatMap((item) => item.predicate.nodeId === "relation.projection.predicate.presents"
    && item.object.kind === "ref" && targets.has(item.object.target.nodeId) ? [[item.object.target.nodeId, item]] : []));
  ...
  const presentations = facts.map((node) => {
    ...
    return reference(`presents:${target_id}`, "relation.projection.predicate.presents", target_id, [
      constant(`frame:${target_id}`, "relation.projection.predicate.frame", next_frame),
    ]);
  });
  return { ...source, relations: [
    ...source.relations.filter((item) => !["relation.projection.predicate.presents", "relation.projection.predicate.child-predicate"].includes(item.predicate.nodeId)),
    reference("child-predicate", "relation.projection.predicate.child-predicate", "spot.terminal.predicate.contains"), ...presentations,
  ] };
};

const put_changed = (operations, graph, node) => {
  if (!same(graph.nodes[node.id], node)) operations.push({ op: "put-node", node });
};
const ensure_projection = (operations, graph, node) => {
  const current = graph.nodes[node.id];
  if (!current) return operations.push({ op: "put-node", node });
  const authority = new Set(["relation.core.identity", "relation.core.type", "relation.projection.predicate.observes", "relation.projection.predicate.uses"]);
  const merged = { ...current, relations: [...current.relations.filter((item) => !authority.has(item.predicate.nodeId)), ...node.relations] };
  if (!same(current, merged)) operations.push({ op: "put-node", node: merged });
};
...
export function reconcile_facts(graph, terminal_id, state) {
  ...
  const terminal = graph.nodes[terminal_id];
  const next_terminal = { ...terminal, relations: [
    ...terminal.relations.filter((item) => item.predicate.nodeId !== "spot.terminal.predicate.contains"),
  ...
  if (flow) put_changed(operations, graph, composed_projection(flow, facts, "flow"));
  if (world) put_changed(operations, graph, composed_projection(world, facts, "world"));
  const stale_ids = Object.keys(graph.nodes).filter((id) => id.startsWith(`${terminal_id}:fact:`) && !desired_ids.has(id));
  for (const node_id of stale_ids) operations.push({ op: "remove-node", nodeId: node_id });
