...
const ports = (relations, includeBinding) => relations.map((relation) => ({
  ...portMeta(relation), ...(includeBinding && bindingOf(relation) ? { binding: binding(bindingOf(relation)) } : {}),
  ...(constOf(nestedBy(relation, FLOW.delay)) === true ? { delayBoundary: true } : {}),
}));
...
function ranks(children) {
  ...
  for (const child of children) for (const input of flowInputs(child.node)) {
    ...
    if (source?.kind === "ref" && ids.has(source.target.nodeId) && source.target.nodeId !== child.node.id && constOf(nestedBy(input, FLOW.delay)) !== true) dependencies.get(child.node.id).add(source.target.nodeId);
  }
  ...
  while (pending.size) {
    ...
    ready.forEach((id) => { result.set(id, rank); pending.delete(id); }); rank += 1;
  }
  return result;
}

function compileScope(node, graph, ancestors = new Set()) {
  if (ancestors.has(node.id)) throw new Error(`Flow contains cycle reaches ${node.id}`);
  const nextAncestors = new Set(ancestors).add(node.id);
  const children = flowChildren(node).map((ref) => ({ ref, node: graph.nodes[ref.nodeId] })).filter((item) => item.node);
...
export const flowPlanner = {
  id: "relation.flow.planner",
  matches(node) { return flowInputs(node).length > 0 || flowOutputs(node).length > 0 || flowChildren(node).length > 0; },
  compile(node, graph) { return { plannerId: this.id, rootNodeId: node.id, graphRevision: graph.revision, root: compileScope(node, graph) }; },
};
