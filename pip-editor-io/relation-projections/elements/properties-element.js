...
export class RelationPropertiesElement extends HTMLElement {
  ...
  render() {
    ...
    const portSide = (ports, direction) => `<div class="side">${ports.map((port) => `<button data-flow-${direction} data-node="${esc(port.nodeId)}" data-relation="${esc(port.id)}">${esc(port.label)}</button>`).join("")}</div>`;
    ...
    this.shadowRoot.querySelectorAll("[data-flow-output]").forEach((button) => button.addEventListener("pointerdown", (event) => {
      event.stopPropagation(); this.dispatchEvent(new CustomEvent("relation-flow-drag-start", { bubbles: true, composed: true, detail: { nodeId: button.dataset.node, relationId: button.dataset.relation } }));
    }));
    this.shadowRoot.querySelectorAll("[data-flow-input]").forEach((button) => {
      button.addEventListener("pointerup", (event) => { event.stopPropagation(); this.dispatchEvent(new CustomEvent("relation-flow-port-drop", { bubbles: true, composed: true, detail: { nodeId: button.dataset.node, relationId: button.dataset.relation } })); });
      button.addEventListener("dblclick", () => this.dispatchEvent(new CustomEvent("intent-relation-request", { bubbles: true, composed: true, detail: { kind: "command", commandId: "relation.flow.disconnect", input: { targetNodeId: button.dataset.node, targetRelationId: button.dataset.relation } } })));
    });
  }
}
