...
import { UNLIMITED_PIP_IO_POLICY, type PipPackageRef } from "../../pip-editor/pip-package/index.ts";
import type { ElementPluginManifest, NodeMapManifest, NodeTypePluginManifest } from "../../pip-editor/pip-host/contracts/package-types.ts";
...
const common = (packageId: string, artifactName: string, name: string, rootNodeId: string) => ({
  ...
  providedEditorKinds: [], supportedDocumentKinds: ["pip-graph/1"], preferredEditorKinds: ["pip-graph/1"],
...
export const baseElementManifest = (id: string, name: string, sourcePaths = ["source/index.js"]): ElementPluginManifest => ({
  ...common(id, `${artifact(id)}_element`, name, id), layer: "a3", elementAbi: "pip-element/2",
  entry: "entry.mjs", elements: [], permissions: [], sourcePaths, sourceSha256: "0".repeat(64), entrySha256: "0".repeat(64),
  redistributable: true, providedCapabilities: ["pip-element/2"],
});
...
export const baseNodeTypeManifest = (id: string, name: string, typeNodeIds: string[], dependencies: PipPackageRef[], sourcePaths = ["source/index.js"]): NodeTypePluginManifest => ({
  ...common(id, `${artifact(id)}_node_type`, name, typeNodeIds[0]), layer: "a4", nodeTypeAbi: "pip-node-type/2",
  entry: "entry.mjs", typeNodeIds, dependencies, permissions: [], sourcePaths, sourceSha256: "0".repeat(64), entrySha256: "0".repeat(64),
  redistributable: true, providedCapabilities: ["pip-node-type/2"], requiredCapabilities: ["pip-element/2"],
});
...
export const baseNodeMapManifest = (id: string, name: string, rootNodeIds: string[], dependencies: PipPackageRef[], packageRootNodeId = rootNodeIds[0] ?? id): NodeMapManifest => ({
  ...common(id, `${artifact(id)}_map`, name, packageRootNodeId), layer: "a5", nodeMapAbi: "pip-node-map/1",
  rootNodeIds, dependencies, requiredCapabilities: ["pip-node-type/2"],
});
