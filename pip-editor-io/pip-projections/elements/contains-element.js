...
const request = (host, detail) => host.dispatchEvent(new CustomEvent("intent-pip-request", { bubbles: true, composed: true, detail }));

export class PipContainsElement extends HTMLElement {
  constructor() {
    ...
    this.addEventListener("pip-flow-drag-start", (event) => { this.flowSource = event.detail; });
    this.addEventListener("pip-flow-port-drop", (event) => this.connect(event.detail));
  }
  ...
  render() {
    ...
    const ports = (items, direction) => items.map((item) => `<button class="port" data-flow-${direction} data-node="${esc(item.nodeId)}" data-pip="${esc(item.id)}">${esc(item.label)}</button>`).join("");
    ...
    request(this, { kind: "command", commandId: "pip.move-child-projection", input: { parentProjectionId: data.projectionNodeId, childProjectionId: drag.projectionNodeId, frame } });
  }
    bindFlow(data, state) {
        ...
        this.shadowRoot.querySelectorAll("[data-flow-output]").forEach((button) => button.addEventListener("pointerdown", (event) => { event.stopPropagation(); this.flowSource = { node_id: button.dataset.node, pip_id: button.dataset.pip }; }));
        this.shadowRoot.querySelectorAll("[data-flow-input]").forEach((button) => {
            button.addEventListener("pointerup", (event) => { event.stopPropagation(); this.connect({ node_id: button.dataset.node, pip_id: button.dataset.pip }); });
            button.addEventListener("dblclick", () => request(this, { kind: "command", commandId: "pip.flow.disconnect", input: { targetNodeId: button.dataset.node, target_pip_id: button.dataset.pip } }));
        });
    }
    renderEdges(data, session) {
        ...
        const boundary = (ref, output) => {
            ...
            const button = [...this.shadowRoot.querySelectorAll(selector)].find((item) => item.dataset.node === ref.node_id && item.dataset.pip === ref.pip_id);
            if (!button || !world)
                return { x: output ? 4 : this.clientWidth - 4, y: 54 };
            const outer = world.getBoundingClientRect(), inner = button.getBoundingClientRect();
            return screenToElementLocal({ x: inner.left + inner.width / 2, y: inner.top + inner.height / 2 }, outer, { width: world.offsetWidth, height: world.offsetHeight });
        };
        const position = (ref, output) => {
            const item = items.get(ref.node_id);
            if (!item)
                return boundary(ref, output);
            const frame = this.previewFrames.get(item.projectionNodeId) ?? item.frame;
            const list = output ? item.outputs : item.inputs, index = Math.max(0, list.findIndex((port) => port.id === ref.pip_id));
        ...
        svg.innerHTML = (data.flow?.edges ?? []).map((edge) => {
            const from = position(edge.source, true), to = position(edge.target, false), bend = (from.x + to.x) / 2, live = liveNodes.has(edge.source.node_id) || liveNodes.has(edge.target.node_id);
    ...
    connect(target) {
        if (!this.flowSource)
            return;
        const source = this.flowSource;
        this.flowSource = null;
        request(this, { kind: "command", commandId: "pip.flow.connect", input: { sourceNodeId: source.node_id, source_pip_id: source.pip_id, targetNodeId: target.node_id, target_pip_id: target.pip_id } });
    }
    openAt(x, y) {
    ...
    request(this, { kind: "command", commandId: "pip.attach-child-projection", input: { parentProjectionId: data.projectionNodeId, childProjectionId, frame } });
