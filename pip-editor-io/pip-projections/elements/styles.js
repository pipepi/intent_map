...
.compact .bar{cursor:move}.compact .bar strong{min-width:0;flex:1}.compact .bar .muted,.compact .bar button{flex:none;white-space:nowrap}.compact .bar button{cursor:pointer}.compact .enter{margin-left:0}
.compact-ports{display:flex;justify-content:space-between;gap:8px;padding:5px 8px}.compact-ports .side{display:flex;flex-direction:column;gap:4px}.compact-ports button{font-size:10px;padding:2px 5px;cursor:crosshair}
`;
export const contains = `${shared}
:host{position:relative;overflow:clip}.surface{position:relative;height:calc(100% - 34px);min-height:900px;overflow:clip;background-image:linear-gradient(#24345b44 1px,transparent 1px),linear-gradient(90deg,#24345b44 1px,transparent 1px);background-size:28px 28px}.world{position:absolute;inset:0;min-height:900px;transform:translate(var(--projection-pan-x,0),var(--projection-pan-y,0)) scale(var(--projection-zoom,1));transform-origin:top left;transition:transform 90ms ease-out}
