...
const genericTarget = (node, predicate) => {
  const object = node?.relations.find((relation) => relation.predicate.nodeId === predicate)?.object;
  return object?.kind === "ref" ? object.target : undefined;
};
export const observedScene = (view, graph) => graph.nodes[genericTarget(view, "relation.projection.predicate.observes")?.nodeId];
...
export const projectionId = (view) => genericTarget(view, "relation.projection.predicate.uses")?.nodeId;
