...
const controls = (data) => {
  ...
  return `${spatial}${control("xZoom", "x 缩放", camera.xZoom, .5, 3, .1, "×")}${control("xPan", "x 平移", camera.xPan, -250, 250, 10)}<div class="counts">${data.counts.entities} 个实体 · ${data.counts.events} 个事件 · ${data.counts.pips} 条关系</div>`;
};
const axes = (data) => {
  if (!data.axes) return "";
...
const svg = (data) => `<svg class="canvas" viewBox="0 0 960 540" role="img" aria-label="${data.mode === "quadrant" ? "象限" : "管道"}时空观察图">
  ...
  <g>${data.entityLines.map((line) => `<line class="entityPip" x1="${line.from.x}" y1="${line.from.y}" x2="${line.to.x}" y2="${line.to.y}"/>`).join("")}${data.pipLines.map((line) => `<line class="eventPip ${line.active ? "active" : ""}" x1="${line.from.x}" y1="${line.from.y}" x2="${line.to.x}" y2="${line.to.y}"/>`).join("")}</g>
...
const inspector = (data) => {
  ...
  return `<aside class="inspector"><p class="eyebrow">${item.kind === "event" ? "SELECTED EVENT" : "SELECTED NODE"}</p><h3>${esc(item.name)}</h3><p class="id">${esc(item.id)}</p><div class="detail"><label>${item.kind === "event" ? "时间" : "类型"}</label><strong>${esc(detail)}</strong><small>${item.kind === "event" ? "事件位置由关系目标的几何中心实时计算" : "实体世界位置会被两个投影共同观察"}</small></div><div class="pips">${item.pips.map((pip) => `<div class="pip"><span>${esc(pip.role)}</span><strong>${esc(pip.name)}</strong><small>${esc(pip.kind)}</small></div>`).join("")}</div><div class="rule">event.yz = average(pips[].node.yz)</div></aside>`;
};
const formatHour = (hour) => `${String(Math.floor(hour)).padStart(2, "0")}:${String(Math.round((hour % 1) * 60)).padStart(2, "0")}`;
export const renderView = (data) => `<style>${styles}</style><section class="panel"><header class="header" data-window-drag><div><p class="eyebrow">SCENE / EVENT OBSERVER</p><h2>小明的今天</h2></div><span class="mode">${data.mode === "quadrant" ? "单象限" : "管道"}</span></header><p class="story"><span>今天</span>小明 08:00 从家出发并送早餐；随后购买 BTC 和烤鸡。</p><div class="workspace"><section class="stage"><div class="controls">${controls(data)}</div><div class="canvasViewport" data-projection-surface>${svg(data)}</div><div class="legend"><span>○ 实体</span><span>● 事件</span><span>┄ 关系</span></div></section>${inspector(data)}</div></section>`;
