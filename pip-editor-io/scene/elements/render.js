...
export const renderView = (data, frame) => `<style>${styles}</style><section class="panel"><header class="header" data-window-drag><div><p class="eyebrow">SCENE / EVENT OBSERVER</p><h2>小明的今天</h2></div><div class="windowActions"><span class="mode">${data.mode === "quadrant" ? "单象限" : "管道"}</span><button data-resize-toggle title="${frame?.resizeMode === "full" ? "四边四角 · 点击切换为三向" : "右边、下边、右下角 · 点击切换为八向"}">${frame?.resizeMode === "full" ? "⤢" : "┘"}</button></div></header><p class="story"><span>今天</span>小明 08:00 从家出发并送早餐；随后购买 BTC 和烤鸡。</p><div class="workspace"><section class="stage"><div class="controls">${controls(data)}</div>${svg(data)}<div class="legend"><span>○ 实体</span><span>● 事件</span><span>┄ 关系</span></div></section>${inspector(data)}</div></section>`;
